...
const LoginModal = ({ open, onClose }) => {
  const dispatch = useDispatch(); // if we have to send anything in store we have to use dispatch
  const handleGoogleAuth = async () => {
    try {
      ...
      const { data } = await axios.post(
        ...
        {
          // backend ke authController.js mein ye 3 chiz define hai
          name: result.user.displayName,
          email: result.user.email,
          avatar: result.user.photoURL,
        },
        { withCredentials: true },
      ); // withCredentials: true isliye use kiya hai kyuki hum cookies ko send kar rhe hai
      dispatch(setUserData(data)); // redux store mein user data ko save kar rhe hai
      onClose(); // modal ko close kar rhe hai
    } catch (error) {
      console.log(error);
    }
  };
    ...
    <div>
      {open && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          onClick={onClose} // on clicing anywhere outside it will get close
